...
import { CExp, isPrimOp, PrimOp, VarDecl, unparse, ProcExp } from './L4-ast';
...
export const makeTracedClosure = (closure: Closure, name: string): TracedClosure =>
    ({tag: "TracedClosure", closure, name, counter: 0});

export const increaseCounter = (tracedClosure: TracedClosure): void => {
    const temp: number = tracedClosure.counter + 1;
    tracedClosure.counter = temp;
}

export const decreaseCounter = (tracedClosure: TracedClosure): void => {
    const temp: number = tracedClosure.counter - 1;
    tracedClosure.counter = temp;
}
