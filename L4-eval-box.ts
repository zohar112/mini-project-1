...
import { isClosure, makeClosure, Closure, Value, valueToString, TracedClosure, isTraceClosure, makeTracedClosure, increaseCounter, decreaseCounter } from "./L4-value-box";
...
const evalTraceExp = (exp: TraceExp, env: Env): Result<void> => 
    bind(applyEnvBdg(env,exp.var.var),(x: FBinding)=> {
        let v: Value = getFBindingVal(x);
        return isClosure(v) ? makeOk(setFBinding(x,makeTracedClosure(v,exp.var.var))) : makeFailure("not valid variable");
    });
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    printPreTrace(proc.name,args,proc.counter);
    increaseCounter(proc);
    const val:Result<Value> = applyClosure(proc.closure,args);
    decreaseCounter(proc);
    isOk(val) ? printPostTrace(val.value,proc.counter) : makeFailure("");
    return val;
}
